...
def build_toc(sections, lang="cn"):
    ...
    items = []
    for num, _key, en_title, cn_title, _ in sections:
        title = cn_title if lang == "cn" else en_title
        anchor = f"section-{str(num).zfill(2)}"
        items.append(f'<li class="toc-part"><a href="#{anchor}" style="text-decoration:none;color:inherit;">{title}</a></li>')
...
def build_single(md_text, name, lagna, gender, status, lang="cn"):
    """Single-file mode: wrap one analysis.md as a complete HTML report."""
    pkg = "完整报告" if lang == "cn" else "Complete Report"
    desc = pkg
    cover = build_cover(name, lagna, gender, status, pkg, desc, lang)
    toc_title = "目录" if lang == "cn" else "Table of Contents"
    ...
    for line in md_text.split("\n"):
        if line.startswith("## "):
            if current_title is not None:
                anchor = f"section-{sec_num-1:02d}"
                body = markdown.markdown("\n".join(current_section_lines), extensions=["tables", "fenced_code"])
                sections_html.append(f'<div class="section" id="{anchor}"><div class="section-header">'
                                     f'<div class="section-number">Section {sec_num-1:02d}</div>'
                                     f'<h2>{current_title}</h2></div>{body}</div>')
            current_title = line[3:].strip()
            anchor = f"section-{sec_num:02d}"
            toc_items.append(f'<li class="toc-part"><a href="#{anchor}" style="text-decoration:none;color:inherit;">{current_title}</a></li>')
            current_section_lines = []
            sec_num += 1
        else:
            if current_title is not None:
    ...
    toc = f'<div class="toc"><h2>{toc_title}</h2><ul class="toc-list">{"".join(toc_items)}</ul></div>'
    footer = '<div class="footer-note">本报告基于传统吠陀占星方法（Parashari Jyotish | KN Rao School）。</div>'
    html_lang = "zh-CN" if lang == "cn" else "en"
    return (f'<!DOCTYPE html><html lang="{html_lang}"><head><meta charset="UTF-8">'
            f'<title>Vedic Report — {name}</title><style>{CSS}</style></head>'
            f'<body>{cover}{toc}{"".join(sections_html)}{footer}</body></html>')
...
def main():
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  python report_builder.py ./folder --name "Client" --lang cn
  python report_builder.py ./folder --single analysis.md --name "Client"
        """)
    parser.add_argument("folder", help="Folder with MD files")
    ...
    parser.add_argument("--single", default=None, help="Single markdown file (e.g. analysis.md)")
    ...
    single_path = None
    if args.single:
        single_path = os.path.join(folder, args.single)
    else:
        auto = os.path.join(folder, "analysis.md")
        if os.path.exists(auto):
            single_path = auto

    if single_path and os.path.exists(single_path):
        print(f"  Single-file mode: {single_path}")
        with open(single_path, "r", encoding="utf-8") as f:
            content = f.read()
        html = build_single(content, args.name, args.lagna, args.gender, args.status, lang)
